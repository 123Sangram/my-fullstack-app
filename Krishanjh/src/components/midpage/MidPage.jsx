...
const MidPage = () => {
  ...
  return (
    <div className="min-h-[500px] bg-[#E5FFEF] relative">
      <div className="absolute w-full overflow-hidden mt-[-180px] px-4 sm:px-10 lg:px-20">
        {/* ✅ Grid container */}
          ...
          {items.map((item, index) => (
            ...
            >
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: index * 0.2 }}
                className="bg-[#243F32] rounded-[15px] h-[350px] sm:h-[400px] lg:h-[450px] border border-[#bbbfbd] shadow-lg p-4 cursor-pointer flex flex-col justify-between"
              >
                <p className="mt-2 text-[18px] sm:text-[20px] md:text-[21px] text-center text-white font-semibold">
                  {item.title}
                </p>
                <img
                  src={item.src}
                  alt={item.title}
                  className="w-[80px] sm:w-[90px] md:w-[100px] h-[80px] sm:h-[90px] md:h-[100px] mx-auto mt-[-40px] object-contain"
                />
                <p className="mb-[30px] text-[12px] sm:text-[14px] md:text-[15px] text-center text-white">
                  {item.description}
                </p>
              </motion.div>
            </motion.div>
